...
| CI | Manual Release build | Push/PR/manual builds, Debug/Release, lint, sanitizers, static analysis, archive validation and Android runtime tests | Official actions pinned to release commits; all host and Android checks must pass before publishing the single artifact |
...
Every successful workflow now publishes exactly one artifact, `zygisk.zip`, after both host and Android checks pass. Test diagnostics remain in the workflow logs.
